...
export class EditAccountComponent {
    ...
    ngOnInit() {
        ...
        if (localStorage.getItem('userInfo') != null && localStorage.getItem('userInfo') != undefined && localStorage.getItem('userInfo') != "undefined") {
            ...
            debugger;
            this.model.dept = 0;
            this.model.role_id = this.userData.access_permission_info.role_id;
            this.model.firstname = this.userData.user_account_info.firstname;
            this.model.lastname = this.userData.user_account_info.lastname;
            this.apiurl = this.userData.user_account_info.profile_pic;
            this.apiurl1 = this.userData.user_account_info.profile_pic;
            this.model.gender = this.userData.user_account_info.gender;
    ...
    onSubmit(model) {
        ...
        model.profile_pic = this.apiurl1;
        //var body = JSON.stringify(model);
        debugger;
        this.accountsService.editAccount(model)
            .subscribe(
            result => {
                this.result = result;
                this.resetModel()
                jQuery("#btnSubmit").html("Edit Profile").prop('disabled', false);
            },
            error => this.error = <any>error);
